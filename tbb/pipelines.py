...
class TbbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('tbb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `tbb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM tbb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO tbb (date, title, link, content)"
